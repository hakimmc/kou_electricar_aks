...
extern int sd_card_spi_write(uint8_t *data, size_t len);
extern int sd_card_spi_read(uint8_t *data, size_t len);
extern void sd_card_select(void);
extern void sd_card_deselect(void);

static
void xmit_spi(BYTE Data)
{
  sd_card_spi_write(&Data, 1);
}

static BYTE rcvr_spi(void)
{
  BYTE b;
  sd_card_spi_read(&b, 1);
...
void rcvr_spi_m(BYTE *dst) {
	sd_card_spi_read(dst, 1);
}

static
void rcvr_spi_array(BYTE *dst, UINT len) {
  sd_card_spi_read(dst, len);
}
